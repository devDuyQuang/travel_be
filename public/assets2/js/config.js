...
const currentTemplateName = htmlEl.getAttribute('data-template');
if (typeof window.Helpers !== 'undefined') {
  window.Helpers.setColor('#0d9394', true);
}

if (
  typeof TemplateCustomizer !== 'undefined' &&
  typeof window.Helpers !== 'undefined' &&
  currentTemplateName &&
  !htmlEl.classList.contains('customizer-hide')
) {
  window.templateCustomizer = new TemplateCustomizer({
    displayCustomizer: true,
    lang: localStorage.getItem('templateCustomizer-' + currentTemplateName + '--Lang') || 'en',
    defaultPrimaryColor: '#0d9394',
    controls: [
      'color',
      'theme',
      'skins',

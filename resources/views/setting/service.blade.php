@extends('index')
@section('title', 'Cấu hình trực tuyến - Dịch Vụ')
...
<div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
  <h5 class="mb-0 d-flex align-items-center gap-2 flex-wrap"> <i class="icon-base ti tabler-server-spark"></i>Cấu hình trang Dịch vụ</h5>
  <div class="d-flex align-items-center gap-2">
    ...
    <div class="btn-group" role="group" aria-label="Chọn loại domain">
      @foreach($settingTypes as $typeKey => $typeLabel)
        <a href="{{ $baseServiceUrl . '?type=' . $typeKey . '&section=' . $currentSection }}"
      ...
      <div class="tab-content" id="home-sections-tabContent">
        @foreach($sectionsList as $item)
          ...
          <div class="tab-pane fade {{ $currentSection === $key ? 'show active' : '' }}"
            ...
            @includeIf('setting.service.sections.' . $key, $data)
          </div>
        @endforeach
      </div>

  ...
  <div class="main-content">
    ...
    <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
      <h5 class="mb-0 d-flex align-items-center gap-2 flex-wrap">
        <i class="icon-base ti tabler-user-screen"></i>
      ...
      <div class="d-flex align-items-center gap-2">
        ...
        <div class="btn-group" role="group">
          @foreach($settingTypes as $typeKey => $typeLabel)
            <a
              href="{{ $baseAboutUrl . '?type=' . $typeKey . '&section=' . $currentSection }}"
    ...
    <div class="card-body text-start">
      ...
      <div class="row g-4 justify-content-start setting-layout-row">
        ...
        <div class="col-12 col-lg-auto text-start setting-sidebar-col">
          <div
            class="nav flex-column nav-pills border rounded p-2 text-start setting-sidebar-tabs"
            id="about-pills-tab"
            role="tablist"
            ...
            @foreach($sectionsList as $item)
              ...
              <a
                ...
                <span class="me-1 text-muted">{{ $loop->iteration }}.</span>
                {{ $item['label'] }}
              </a>
            @endforeach
          </div>
        ...
        <div class="col-12 setting-content-col border rounded p-3 setting-content-box">
          <div class="tab-content" id="about-sections-tabContent">
            @foreach($sectionsList as $item)
              ...
              <div
                class="tab-pane fade {{ $currentSection === $key ? 'show active' : '' }}"
          ...
          <div
            id="about-json-wrapper"
            class="border rounded mt-3 p-2 bg-dark-subtle"
            ...
            <div class="d-flex justify-content-between align-items-center mb-1">
              ...
              <button
                ...
                id="btn-clear-about-json">
                Xoá
              </button>
            </div>

            <pre
              id="about-json-preview"
              class="bg-black text-white small rounded p-2 mb-0"
              style="max-height: 260px; overflow: auto; white-space: pre-wrap; word-break: break-all;"></pre>
          </div>
        </div>
      </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function getDbJsonMap() {
    try {
      const dbJsonRaw = wrapper ? wrapper.getAttribute('data-db-json') : null;
      return dbJsonRaw ? JSON.parse(dbJsonRaw) : {};
    } catch (e) {
      return {};
    }
  }

  function showJsonBySection(sectionKey) {
    if (!pre || !wrapper) return;

    const key = sectionKey ? `about_json_${type}_${sectionKey}` : `about_json_${type}_preview`;
    storageKey = key;

    try {
      const savedJson = window.localStorage ? window.localStorage.getItem(key) : null;
      const dbJsonMap = getDbJsonMap();

      const displayData = savedJson || (
        dbJsonMap[sectionKey] ? JSON.stringify(dbJsonMap[sectionKey], null, 2) : null
      );
      ...
      updateTypeLinks(sectionKey || 'hero');
    } catch (e) {
      console.warn('Không thể load JSON', sectionKey, e);
    }
  }

  updateTypeLinks(section || 'hero');
  showJsonBySection(section);
  ...
  document.querySelectorAll('#about-pills-tab .nav-link[data-section]').forEach(function (link) {
